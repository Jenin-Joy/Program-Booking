...
if(isset($_POST["save"]))
{
	...
	$troop=$_SESSION['troopid'];
	$date=$_POST["txtdate"];
	$pgmloc=$_POST["txtpgmlocation"];
	$co_condact=$_POST["txtcondact"];
	$amt=$_GET["artistrate"];
	//echo $date;
	$ins="insert into tbl_artistbook(artist_id,artistbook_date,artistbook_todate,arbook_totalamt,condact,pgm_loc,troup_id) value('".$artist."','".date("Y-m-d")."','".$date."','".$amt."','".$co_condact."','".$pgmloc."','".$troop."')";
	mysql_query($ins);
...
?>
  ...
  <tr>
    ...
    <tr>
      ...
      <textarea name="txtpgmlocation" id="txtpgmlocation" cols="35" rows="5" required="required" placeholder="Program Location"></textarea></td>
    </tr>
    <tr>
      <td>Co-ordinator Condact</td>
      <td><label for="txtcondact"></label>
      <input type="text" name="txtcondact" id="txtcondact" required="required" autocomplete="off" placeholder="Condact Number" pattern="[0-9]{10,12}" /></td>
    </tr>
    <tr>
      <td>Amount</td>
      <td><input type="text" name="rate" readonly="readonly" value="<?php echo $row["artist_rate"] ?>"/></td>
    </tr>
    <tr>
